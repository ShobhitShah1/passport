...
export const usePasswordManager = () => {
  ...
  const copyWithFeedback = useCallback(async (text: string, label: string = 'Text') => {
    try {
  ...
  const savePasswordEntry = useCallback(async (passwordData: {
    appName: string;
    appId?: string;
    username: string;
    email: string;
  ...
  const searchPasswordsAdvanced = useCallback((query: string, filters?: {
    ...
    return passwords.filter(password => {
      const matches = [
        ...
        includeUsernames && password.username?.toLowerCase().includes(lowercaseQuery),
        includeEmails && password.email?.toLowerCase().includes(lowercaseQuery),
        includeUrls && password.url?.toLowerCase().includes(lowercaseQuery),
        includeNotes && password.notes?.toLowerCase().includes(lowercaseQuery),
        includeTags && password.tags.some(tag => tag.toLowerCase().includes(lowercaseQuery)),
      ];
  ...
  const getRecentPasswords = useCallback((limit: number = 6) => {
    return passwords
  ...
  const updatePasswordWithUsage = useCallback(async (id: string, updates: Partial<PasswordEntry>) => {
    try {
  ...
  const copyUsername = useCallback(async (password: PasswordEntry) => {
  ...
  const copyEmail = useCallback(async (password: PasswordEntry) => {
  ...
  const copyPasswordField = useCallback(async (password: PasswordEntry) => {
  ...
  const copyUrl = useCallback(async (password: PasswordEntry) => {
    if (password.url) {
      await updatePasswordWithUsage(password.id, {});
