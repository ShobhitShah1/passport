...
export async function getInstalledApps(): Promise<InstalledApp[]> {
  try {
    ...
    if (typeof getAppList !== 'function') {
      console.error('react-native-get-app-list not properly installed or linked');
    ...
    console.log(`✅ Successfully retrieved ${installedApps.length} installed apps`);
    
    if (installedApps.length > 0) {
      console.log('📱 Sample app data:', JSON.stringify(installedApps[0], null, 2));
    }
...
function processApps(nativeApps: NativeAppInfo[]): InstalledApp[] {
  ...
  const filteredApps = matchedApps.filter(app => {
    ...
    const isSystemApp = systemPackages.some(pkg => app.packageName.toLowerCase().startsWith(pkg.toLowerCase()));
    ...
    const systemKeywords = ['service', 'framework', 'system', 'config', 'stub', 'overlay', 'launcher'];
    const nameContainsSystemKeyword = systemKeywords.some(keyword => 
...
function findMatchingApp(nativeApp: NativeAppInfo): AppData | undefined {
  ...
  let match = POPULAR_APPS.find(app => 
    (Platform.OS === 'android' && app.packageName === nativeApp.packageName) ||
  ...
  if (!match) {
    ...
    match = POPULAR_APPS.find(app => {
      ...
      return cleanName.includes(appCleanName) || appCleanName.includes(cleanName);
    });
  }
